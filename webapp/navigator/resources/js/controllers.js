angular.module('SunNavigator.controllers', [])
  ...
  .controller('NavigatorCtrl', function($http, $scope, $rootScope, $routeParams, $q, DataProvider, chapterUserdataArr) {
        //currentChapter
        $scope.currentChapter = DataProvider.materialMap[$routeParams.cid];
       // TODO:给一切元数据进行排序
        //sort for layers of chapter

        //sort for lessons of layer of chapter


        
        //get all the lessons userdata of the chapter -- resolve
        //因为在navigator的区域没有了“复习”的选项，所以就没有了获取lesson material的需求了     
        var chapterUserdataMap = DataProvider.chapterUserdataMap[$routeParams.cid];
        ...
        if(!chapterUserdataMap) {
            ...
            chapterUserdataMap = {};
            _.each($scope.currentChapter.layers, function(layer, index) {
                _.each(layer.lessons, function(lesson, index) {
            ...
            DataProvider.chapterUserdataMap[$routeParams.cid] = chapterUserdataMap;            
        }
        
        $scope.chapterUserdata = chapterUserdataMap;


        $scope.isUnlocked = function(lessonItem) {
            if(!lessonItem.requirements) {
                return true;
            }

            _.each(lessonItem.requirements, function(item, index) {
                if(!chapterLessonStateMap[item.id]) return false;
            })

            return true;
        }

        $scope.clickOnLayer = function(layer) {
            //如果没有解锁那么就不响应点击（或者不可点击）
            if($scope.layerIsUnlocked(layer)) {
                //展开对应的layer-->手风琴
                
            }
        }

        $scope.layerIsUnlocked = function(layer) {
            //查看当前layer的上一个layer的main lessons是否全部complete
            _.each($scope.currentChapter.layers, function(itemLayer, layerIndex) {
                if(itemLayer.id == layer.id) {
                    if(layerIndex != 0 && layerIndex>=1) {
                        var preLayer = $scope.currentChapter.layers[layerIndex-1];
                        _.each(preLayer.lessons, function(itemLesson, lessonIndex) {
                            if(itemLesson.isMain && !chapterLessonStateMap[itemLesson.id]) {
                                return false;
                            }
                        })
                        return true;
